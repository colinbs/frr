...
def no_test_modify_and_delete_router_id(request):
    ...
    # Modify router id
    input_dict = {
        "r1": {"bgp": {"router_id": "10.0.0.2"}},
        "r2": {"bgp": {"router_id": "10.0.0.3"}},
    }
...
def no_test_bgp_config_with_4byte_as_number(request):
    """
    ...
    input_dict = {
        "r1": {"bgp": {"local_as": 65536}},
        "r2": {"bgp": {"local_as": 65537}},
    }
...
def no_test_BGP_config_with_invalid_ASN_p2(request):
    """
...
def no_test_BGP_config_with_2byteAS_and_4byteAS_number_p1(request):
    """
...
def no_test_bgp_timers_functionality(request):
    """
...
def no_test_bgpsec_convergence(request):
    tgen = get_topogen()
    ...
    test_func = functools.partial(_bgp_converge, router, "10.0.0.2", 4)
    ...
    test_func = functools.partial(_bgp_converge, router, "10.0.0.1", 2)
...
def no_test_static_routes(request):
...
def no_test_admin_distance_for_existing_static_routes(request):
...
def test_advertise_network_using_network_command(request):
            ...
            "bgp": {
                "address_family": {
                    "ipv4": {
                        "unicast": {
                            "advertise_networks": [
                                {"network": "20.0.0.0/32", "no_of_network": 4},
                            ]
                        }
                    }
                }
            ...
            "bgp": {
                "address_family": {
                    "ipv4": {
                        "unicast": {
                            "advertise_networks": [
                                {"network": "30.0.0.0/32", "no_of_network": 2},
                            ]
                        }
                    }
                }
    ...
    test_func = functools.partial(_bgp_converge, router, "10.0.0.2", 2)
    success, result = topotest.run_and_expect(test_func, None, count=1, wait=1)
    ...
    test_func = functools.partial(_bgp_converge, router, "10.0.0.1", 4)
    success, result = topotest.run_and_expect(test_func, None, count=1, wait=1)
...
def no_test_clear_bgp_and_verify(request):
    """
...
def no_test_bgp_with_loopback_interface(request):
    """
